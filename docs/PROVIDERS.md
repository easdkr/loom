...
### `type` (필수)
현재는 `"pty"`와 내장 Claude Code proxy인 `"croxy"`가 의미가 있다. 향후 `"http"` / `"shell"` 등이 추가될 예정.

### `command` / `args`
실행할 바이너리와 인자. PATH에서 찾는다.

> **금지**:
> - `claude --print` (또는 `-p`) — Loom이 강제로 거부한다.
> - `codex exec` — Loom이 강제로 거부한다.
>
> 이유: Loom의 코어 계약은 **interactive PTY**로 우회하되 출력만 headless처럼 보이게 가공하는 것이다. CLI의 headless/print 모드를 직접 호출하면 권한·인증·툴 콜 등 인터랙티브 전용 기능이 빠져 의도와 다른 결과가 나온다. Provider 구성에서 위 인자를 발견하면 즉시 실패 처리한다.
...
### `display_mode`
사용자에게 이 Provider 출력을 어떤 뷰로 보여줄지 결정한다. PTY transport에는 영향을 주지 않으며 `pty:data` / `pty:complete` / `pty:error` 및 `node_write` / `kill` / `resize` 계약은 그대로 유지된다.

- `agent`: 에이전트 응답처럼 가공된 출력 뷰에 최적화
- `terminal`: raw terminal 세션 뷰에 최적화

비워두면 Provider별 기본 정책을 적용한다.
- `shell` 계열 기본값: `terminal`
- AI CLI (`croxy`, `codex`, `cursor`) 기본값: `agent`
